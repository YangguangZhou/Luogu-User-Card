...
const renderNameTitle = (name, color, ccfLevel, title, cardWidth, rightTop, tag) => {
    ...
    const tagSVG = tag?`<defs><filter x="-0.05" y="-0.05" width="1.15" height="1.1" id="tagFilter"><feFlood flood-color="${nameColor}"/><feComposite in="SourceGraphic"/></filter></defs><text filter="url(#tagFilter)" x="${nameLength + (ccfLevel < 3 ? 10 : 28)}" y="-1" fill="white" font-weight="bold" textLength="${tagLength}" font-size="12">${tag}</text>`:``;
...
const renderAboutText = (userType,followerCount,followingCount,ranking,slogan,darkmode=false) => {
    ...
    if(followingCount=="0" && followerCount=="0"){
        line2 = `<g transform="translate(25,30)"><text x="0" y="15" class="about-text">TA还没有关注任何人，还没有人关注TA</text></g>`
    }else if(followingCount!="0" && followerCount=="0"){
        line2 = `<g transform="translate(25,30)"><text x="0" y="15" class="about-text">TA关注了${followingCount}人，还没有人关注TA</text></g>`
    }else if(followingCount=="0" && followerCount!="0"){
