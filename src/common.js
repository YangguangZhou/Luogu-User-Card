...
const renderNameTitle = (name, color, ccfLevel, title, cardWidth, rightTop, tag) => {
    ...
    const tagSVG = tag?`<defs><filter x="-0.05" y="-0.5" width="1.1" height="1.1" id="tagFilter"><feFlood flood-color="${nameColor}"/><feComposite in="SourceGraphic"/></filter></defs><text filter="url(#tagFilter)" x="${nameLength + (ccfLevel < 3 ? 10 : 28)}" y="-1" fill="white" font-weight="bold" textLength="${tagLength}" font-size="12">${tag}</text>`:``;
