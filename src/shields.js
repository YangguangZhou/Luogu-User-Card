...
const renderSVG = (stats, options) => {
	...
	const counts = [...passed, unpassed];
	if (hideInfo) {
		return renderError("用户开启了“完全隐私保护”，获取数据失败");
	}

	const datas = [
		{ label: "暂无评定", color: "#e8e8e8", data: counts[0] },
		{ label: "入门", color: "#fe4c61", data: counts[1] },
		{ label: "普及-", color: "#f39c11", data: counts[2] },
		{ label: "普及", color: "#ffc116", data: counts[3] },
		{ label: "普及+/提高-", color: "#52c41a", data: counts[4] },
		{ label: "提高", color: "#13c2c2", data: counts[5] },
		{ label: "提高+/省选-", color: "#3498db", data: counts[6] },
		{ label: "省选/NOI-", color: "#9d3dcf", data: counts[7] },
		{ label: "NOI/NOI+/CTS", color: "#0e1d69", data: counts[8] },
		{ label: "尝试过的题目", color: "#0101DF", data: counts[9] }
	];
